...
- Module 13：证据支撑的修复项选择
- Module 14：修复授权与实施范围合同
- Module 15：授权修复实施计划

待开发：

- FCPXML 兼容性修复实现（必须等 Module 15 Review 通过）
...
- `prompts/fcpxml_remediation_selection.md`
- `prompts/fcpxml_remediation_authorization.md`
- `prompts/fcpxml_remediation_plan.md`
...
基于 Module 12 `review_ready` 的 compatibility review，由人工选择一个具备 linked evidence 的 remediation item，生成受控修复任务合同。
...
- 只允许从 `review_ready` 的 Module 12 review 中选择
- 必须显式传入单个 `remediation_id`，不得自动选择或批量选择
- selected remediation 必须存在且仍为 `proposed`
- selected finding 必须 `evidence_status="linked"` 且 `evidence_refs` 非空
- selected finding 的 severity 必须是 `blocker` 或 `major`
- 必须从存在的 review JSON 文件计算并冻结 `source_review_sha256`
- `immutable_selection_snapshot` 必须冻结 finding、remediation、evidence refs、related entities、review status、review sha 和选择理由
- `evidence_incomplete`、blocked 或边界违规 review 不得进入选择
- `execution_allowed` 必须固定为 `false`
- `requires_module_14_approval` 必须固定为 `true`
- `task_contract` 必须保持 `implementation_allowed=false`
- `task_contract` 必须保持 `serializer_change_allowed=false`
- 不实施修复，不改 serializer，不改样例，不改 media binding
- 不启动或控制 Final Cut Pro，不自动导入，不读取媒体，不转码、不渲染、不导出成片

运行示例：

```bash
python app/select_fcpxml_remediation.py output/sample_fcpxml_compatibility_review.json output/sample_fcpxml_remediation_selection.json --remediation-id r001 --selected-by module13-reviewer --selected-at 2026-07-04T02:10:00+00:00 --selection-rationale "Select the highest-priority evidence-backed remediation for the next reviewed implementation module."
```

## Module 14：修复授权与实施范围合同

基于 Module 13 selection JSON，生成未来实施修复的授权范围合同。当前阶段只定义 allowed files、prohibited files、verification plan 和 rollback plan，不实施修复。
...
- 必须列出 verification commands 和 rollback steps
...
基于 Module 14 authorization JSON，生成未来实施修复的具体计划。当前阶段只定义 planned file changes、acceptance criteria、review checklist 和 rollback checkpoints，不实施修复。
...
- authorization JSON 文件必须存在，并冻结 `source_authorization_sha256`
...
- authorization 顶层身份、证据、scope、review SHA、verification plan、rollback plan 和执行 flag 必须与 `immutable_authorization_snapshot` 一致；任一不一致都必须阻断，错误码为 `authorization_snapshot_integrity_mismatch`
...
- 每个 `planned_file_changes` 必须包含 `path`、`action` 和 `summary`
- 每个 planned file 必须包含在 Module 14 `allowed_files` 内
- 任何 planned file 不得命中 Module 14 `prohibited_files`
- 必须列出 acceptance criteria、review checklist 和 rollback checkpoints
- acceptance criteria 与 review checklist 不得要求 Final Cut Pro、FFmpeg、OpenCV、moviepy、`osascript` 或媒体处理链路
- `implementation_execution_allowed=false`
- `serializer_change_execution_allowed=false`
- `requires_module_16_approval=true`
- `immutable_plan_snapshot` 必须记录 `verified_authorization_identity`、`verified_verification_plan`、`verified_rollback_plan`、`selected_remediation_id`、`selected_finding_id`、`source_selection_sha256`、`source_review_sha256`、`allowed_files`、`prohibited_files` 和 `authorization_snapshot_verified=true`
...
```bash
python app/plan_fcpxml_remediation.py output/sample_fcpxml_remediation_authorization.json output/sample_fcpxml_remediation_plan.json --planned-by module15-planner --planned-at 2026-07-04T03:15:00+00:00 --planning-rationale "Convert the authorized human-review remediation into a reviewable implementation plan without executing it." --planned-file-change '{"path":"docs/fcpxml_acceptance_record.md","action":"update_manual_review_guidance","summary":"Document the follow-up import with bound online media and required evidence fields."}' --planned-file-change '{"path":"output/sample_fcpxml_acceptance_record_offline_blocked.json","action":"add_manual_follow_up_placeholder","summary":"Reserve a blocked follow-up sample until a human reviewer records online media evidence."}' --acceptance-criterion "Plan references the same authorization SHA-256 used by Module 14." --acceptance-criterion "All planned files are within Module 14 allowed_files." --acceptance-criterion "No serializer, FCPXML generation, editor automation, or media processing is planned." --review-checklist-item "Confirm the implementation plan still requires a later approval module." --review-checklist-item "Confirm rollback checkpoints are explicit before any future implementation." --rollback-checkpoint "Stop before implementation if the authorized remediation no longer matches the manual evidence." --rollback-checkpoint "Discard only future changes made under the implementation module if Review fails."
```
